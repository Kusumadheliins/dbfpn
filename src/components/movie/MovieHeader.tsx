...
function CastList({ cast }: { cast: { name: string; role: string; userId?: number | null; username?: string | null }[] }) {
        ...
        <div className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {currentCast.map((actor, idx) => (
                    <div key={idx} className="bg-[#1a1a1a] border border-gray-800 p-3 rounded-lg flex justify-between items-center group hover:border-primary/50 transition-colors">
                        {actor.username ? (
                            <Link href={`/user/${actor.username}`} className="flex items-center gap-2 text-white font-medium hover:text-primary transition-colors">
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                {!hasNoMedia ? (
                    <div className={`flex flex-col md:flex-row gap-8 ${isSingleMedia ? 'justify-center' : ''} h-auto md:h-[500px]`}>
                        ...
                        {/* Poster */}
                        {hasPoster && (
                            <div className={`relative w-full md:w-auto md:aspect-[2/3] h-[500px] md:h-full rounded-lg overflow-hidden shadow-2xl border-4 border-white/10 shrink-0 ${isSingleMedia ? 'mx-auto' : ''}`}>
                            ...
                            <div className={`relative w-full ${isSingleMedia ? 'md:w-[800px]' : 'md:flex-1'} h-[300px] md:h-full rounded-lg overflow-hidden bg-black border border-gray-800`}>
                                {isPlayingTrailer && trailerId ? (
                                    ...
                                    <div className="relative w-full h-full group cursor-pointer" onClick={() => trailerId && setIsPlayingTrailer(true)}>
                                        {bannerUrl ? (
                                            <Image src={bannerUrl} alt="Backdrop" fill className="object-cover opacity-80 group-hover:opacity-60 transition-opacity" />
                                        ) : (
                                        ...
                                        {trailerId && (
                                            <>
                                                ...
                                                <div className="absolute top-4 left-0 right-0 text-center">
                                                    <h2 className="text-3xl md:text-5xl font-bold text-primary/80 drop-shadow-lg uppercase tracking-widest">{title}</h2>
                                                </div>
                                            </>
                                        )}
                                    </div>
