...
export default async function UserDashboard() {
    ...
    const [
        ...
        prisma.review.count({ where: { userId: Number(user.id) } }),
        prisma.watchlist.count({ where: { userId: Number(user.id) } }),
        prisma.movie.count({ where: { submitterId: Number(user.id) } }),
    ...
    const joinDate = dbUser?.createdAt ? new Date(dbUser.createdAt).toLocaleDateString('id-ID', { month: 'long', year: 'numeric' }) : '-'
        ...
        <>
            ...
            <div className="bg-[#1a1a1a] rounded-xl p-6 border border-gray-800 mb-8 flex items-center justify-between">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-white">{dbUser?.name || user.name || "Pengguna"}</h2>
                        <p className="text-gray-400">@{dbUser?.username || user.email?.split('@')[0]} • Anggota sejak {joinDate}</p>
                        <div className="flex gap-2 mt-3">
                            <span className="px-3 py-1 rounded-full bg-primary/20 text-primary text-xs font-bold border border-primary/20">Anggota {(user as any).role === 'admin' ? 'Admin' : 'Gratis'}</span>
                        </div>
                    </div>
                </div>
                <Link href={`/user/${dbUser?.username || user.id}`} className="px-4 py-2 bg-[#252525] hover:bg-[#333] text-white rounded-lg font-medium transition-colors border border-gray-700">
                    Lihat Profil
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-[#1a1a1a] p-6 rounded-xl border border-gray-800">
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        <div className="text-2xl font-bold text-white">{submissionsCount}</div>
                    </div>
                    ...
                    {recentReviews.length > 0 ? (
                        <div className="space-y-4">
                            {recentReviews.map((review) => (
                                <div key={review.id} className="flex items-start gap-4 pb-4 border-b border-gray-800 last:border-0 last:pb-0">
                                    ...
                                    <div>
                                        ...
                                        {review.content && (
                                            <p className="text-gray-400 text-sm mt-2 italic">&quot;{review.content}&quot;</p>
                                        )}
                                    </div>
                                </div>
                            ))}
